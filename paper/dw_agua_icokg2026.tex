...
becomes \texttt{geo:sfTouches}, and the development index becomes a
...
the point is coarse. Consequently \texttt{geo:sfTouches} expresses proximity
...
SELECT ?neighbour (SUM(?c) AS ?consumption) WHERE {
  agua:colonia/500 geo:sfTouches ?n . ?n rdfs:label ?neighbour .
...
server: the static deployment ships a browser-sized subset ---140,481 triples
using the same vocabulary--- with a SPARQL~1.1 engine running on the client, so a
...
\texttt{geo:sfTouches} is proximity within 1.5~km and not shared boundaries. The
